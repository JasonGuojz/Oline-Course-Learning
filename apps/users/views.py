...
from apps.users.forms import UserInfoForm, ChangePwdForm, UpdateMobileForm, UploadImageForm
...
class MyMessageView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        p = Paginator(messages, per_page=1)
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        current_page = "info"
        captcha_form = RegisterGetForm()  # 修改密码时，需要验证码
...
class LoginView(View):
    def get(self, request, *args, **kwargs):
        ...
        banners = Banner.objects.all()[:3]
        nxt = request.GET.get("next", "")
        login_form = DynamicLoginForm()  # get请求获得验证码
        return render(request, "login.html", {
            "login_form": login_form,
            "next": nxt,
            "banners": banners
    ...
    def post(self, request, *args, **kwargs):
        ...
        if login_form.is_valid():
            ...
            if user is not None:
                ...
                return HttpResponseRedirect(reverse("index"))  # render不改变url，需反向解析url后重新请求
            else:
                # 未查询到用户，将user_name和password展示在login页面
...
class ChangePwdView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if pwd_form.is_valid():
            ...
            user.save()
            # login(request, user)  # 是否直接登陆，根据需求
